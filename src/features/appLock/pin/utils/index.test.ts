import { clearPin, hasPinSet, savePin, verifyStoredPin } from './index';
...
test('savePin으로 새 값을 저장하면 이전 값을 덮어쓴다', async () => {
  await savePin('1234');

  await savePin('5678');

  await expect(verifyStoredPin('1234')).resolves.toBe(false);
  await expect(verifyStoredPin('5678')).resolves.toBe(true);
});
